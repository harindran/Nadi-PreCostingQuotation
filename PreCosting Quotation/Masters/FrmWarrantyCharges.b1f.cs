...
{
    [FormAttribute("PCWCGE", "Masters/FrmWarrantyCharges.b1f")]
    class FrmWarrantyCharges : UserFormBase
    {
        public static SAPbouiCOM.Form objform;
        public FrmWarrantyCharges(string Code)
        {
            try
            {
                if (Code == "") return;
        ...
        private void Form_LoadAfter(SAPbouiCOM.SBOItemEventArg pVal)
        {
            try
            {
                objform = clsModule.objaddon.objapplication.Forms.GetForm("PCWCGE", pVal.FormTypeCount);
