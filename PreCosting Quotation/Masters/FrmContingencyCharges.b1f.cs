...
{
    [FormAttribute("PCCCGE", "Masters/FrmContingencyCharges.b1f")]
    class FrmContingencyCharges : UserFormBase
    {
        public static SAPbouiCOM.Form objform;

        public FrmContingencyCharges(string Code)
        {
            try
            {
                if (Code == "") return;
        ...
        private void Form_LoadAfter(SAPbouiCOM.SBOItemEventArg pVal)
        {
            try
            {
                objform = clsModule.objaddon.objapplication.Forms.GetForm("PCCCGE", pVal.FormTypeCount);
