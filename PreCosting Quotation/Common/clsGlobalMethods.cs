...
namespace PreCosting_Quotation.Common
{
    class clsGlobalMethods
    {
        ...
        public void RemoveLastrow(SAPbouiCOM.Matrix omatrix, string Columname_check)
        {
            try
            {
                if (omatrix.VisualRowCount == 0) return;
                if (string.IsNullOrEmpty(Columname_check.ToString())) return;
        ...
        public void addReport_Layouttype(string FormType, string AddonName)
        {
            ...
            try
            {
                ...
                SAPbobsCOM.ReportTypesParams newtypesParam;
                rptTypeService =(SAPbobsCOM.ReportTypesService)clsModule.objaddon .objcompany.GetCompanyService().GetBusinessService(SAPbobsCOM.ServiceTypes.ReportTypesService);
                newtypesParam = rptTypeService.GetReportTypeList();
                if (clsModule.objaddon. HANA)
                    strsql =clsModule.objaddon.objglobalmethods.getSingleValue("Select 1 as \"Status\" from RTYP Where \"NAME\"='" + FormType + "'");
                else
                    strsql = clsModule.objaddon.objglobalmethods.getSingleValue("Select 1 as Status from RTYP Where NAME='" + FormType + "'");
                ...
                if (ReportExists)
                {
                    ...
                    rptService = (SAPbobsCOM.ReportLayoutsService)clsModule.objaddon.objcompany.GetCompanyService().GetBusinessService(SAPbobsCOM.ServiceTypes.ReportLayoutsService);
                    ...
                    newReport.Author = clsModule.objaddon.objcompany.UserName;
                    ...
                    oBlobParams = (SAPbobsCOM.BlobParams)clsModule.objaddon.objcompany.GetCompanyService().GetDataInterface(SAPbobsCOM.CompanyServiceDataInterfaces.csdiBlobParams);
                    ...
                    int fileSize;
                    fileSize =Convert.ToInt16(oFile.Length);
                    byte[] buf = new byte[fileSize + 1];
                    oFile.Read(buf, 0, fileSize);
                    oFile.Dispose();

                    SAPbobsCOM.Blob oBlob;
                    oBlob =(SAPbobsCOM.Blob) clsModule.objaddon.objcompany.GetCompanyService().GetDataInterface(SAPbobsCOM.CompanyServiceDataInterfaces.csdiBlob);
                    oBlob.Content = Convert.ToBase64String(buf, 0, fileSize);
                    clsModule.objaddon.objcompany.GetCompanyService().SetBlob(oBlobParams, oBlob);
                }
            }
            catch (Exception ex)
            {
                clsModule.objaddon.objapplication.StatusBar.SetText(" addReport_Layouttype Method Failed :  " + ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning);
            }
        }

        public void setReport(string FormUID, string FormType)
        {
            try
            {
                SAPbouiCOM.Form objform;
                objform = clsModule.objaddon.objapplication.Forms.Item(FormUID);
                //objform = clsModule.objaddon.objapplication.Forms.GetForm("SUBCTPO", FormCount);
                SAPbobsCOM.ReportTypesService rptTypeService;
            ...
            {
                clsModule.objaddon.objapplication.StatusBar.SetText("setReport Failed:" + ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning);
            }
        }       

        public void SetCellEditable(SAPbouiCOM.Matrix matrix)
        {
            try
            {
                int matRow = matrix.VisualRowCount;
                for (int i = 1; i < matrix.Columns.Count; i++)
                {
                    if (matrix.CommonSetting.GetCellEditable(matRow, i) == true) continue;
                    matrix.CommonSetting.SetCellEditable(matRow, i, true);
                }
            }
            catch (Exception ex)
            {
